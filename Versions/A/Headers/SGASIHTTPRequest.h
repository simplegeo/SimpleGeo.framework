...
//  http://allseeing-i.com/SGASIHTTPRequest
//
...
@class SGASIDataDecompressor;

extern NSString *SGASIHTTPRequestVersion;
...
typedef enum _SGASIAuthenticationState {
	SGASINoAuthenticationNeededYet = 0,
	SGASIHTTPAuthenticationNeeded = 1,
	SGASIProxyAuthenticationNeeded = 2
} SGASIAuthenticationState;

typedef enum _SGASINetworkErrorType {
    SGASIConnectionFailureErrorType = 1,
    SGASIRequestTimedOutErrorType = 2,
    SGASIAuthenticationErrorType = 3,
    SGASIRequestCancelledErrorType = 4,
    SGASIUnableToCreateRequestErrorType = 5,
    SGASIInternalErrorWhileBuildingRequestType  = 6,
    SGASIInternalErrorWhileApplyingCredentialsType  = 7,
	SGASIFileManagementError = 8,
	SGASITooMuchRedirectionErrorType = 9,
	SGASIUnhandledExceptionError = 10,
	SGASICompressionError = 11
	
} SGASINetworkErrorType;


// The error domain that all errors generated by SGASIHTTPRequest use
...
extern unsigned long const SGASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^SGASIBSGASIcBlock)(void);
typedef void (^SGASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^SGASISizeBlock)(long long size);
typedef void (^SGASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^SGASIDataBlock)(NSData *data);
#endif

@interface SGASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the SGASIHTTPRequestDelegate protocol
	id <SGASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but SGASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <SGASIHTTPRequestDelegate, SGASIProgressDelegate> queue;
	...
	// Automatically set to true in SGASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when SGASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = SGASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <SGASIProgressDelegate> uploadProgressDelegate;
	...
	id <SGASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force SGASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - SGASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	SGASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in SGASINetworkQueue.h
	...
	// Tells SGASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let SGASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// SGASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See SGASIAuthenticationState values above. 0 == default == No authentication needed yet
	SGASIAuthenticationState authenticationNeeded;
	
	// When YES, SGASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops SGASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be SGASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [SGASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <SGASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See SGASICacheDelegate.h for possible values
	SGASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See SGASICacheDelegate.h for possible values
	SGASICacheStoragePolicy cacheStoragePolicy;
	...
	SGASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using SGASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	SGASIHTTPRequest *PACFileRequest;
	...
	SGASIBSGASIcBlock startedBlock;

	//block to execute when headers are received
	SGASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	SGASIBSGASIcBlock completionBlock;

	//block to execute when request fails
	SGASIBSGASIcBlock failureBlock;

	//block for when bytes are received
	SGASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	SGASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	SGASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	SGASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	SGASIDataBlock dataReceivedBlock;

	//block for handling authentication
	SGASIBSGASIcBlock authenticationNeededBlock;

	//block for handling proxy authentication
	SGASIBSGASIcBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    SGASIBSGASIcBlock requestRedirectedBlock;
	#endif
}
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <SGASICacheDelegate>)cache;
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <SGASICacheDelegate>)cache andCachePolicy:(SGASICachePolicy)policy;

#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(SGASIBSGASIcBlock)aStartedBlock;
- (void)setHeadersReceivedBlock:(SGASIHeadersBlock)aReceivedBlock;
- (void)setCompletionBlock:(SGASIBSGASIcBlock)aCompletionBlock;
- (void)setFailedBlock:(SGASIBSGASIcBlock)aFailedBlock;
- (void)setBytesReceivedBlock:(SGASIProgressBlock)aBytesReceivedBlock;
- (void)setBytesSentBlock:(SGASIProgressBlock)aBytesSentBlock;
- (void)setDownloadSizeIncrementedBlock:(SGASISizeBlock) aDownloadSizeIncrementedBlock;
- (void)setUploadSizeIncrementedBlock:(SGASISizeBlock) anUploadSizeIncrementedBlock;
- (void)setDataReceivedBlock:(SGASIDataBlock)aReceivedBlock;
- (void)setAuthenticationNeededBlock:(SGASIBSGASIcBlock)anAuthenticationBlock;
- (void)setProxyAuthenticationNeededBlock:(SGASIBSGASIcBlock)aProxyAuthenticationBlock;
- (void)setRequestRedirectedBlock:(SGASIBSGASIcBlock)aRedirectBlock;
#endif
...
// Used by SGASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (SGASIHTTPRequest *)HEADRequest;
...
// Used internally to record bandwidth use, and by SGASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, SGASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
+ (void)setDefaultCache:(id <SGASICacheDelegate>)cache;
+ (id <SGASICacheDelegate>)defaultCache;
...
// Used for generating Authorization header when using bSGASIc authentication when shouldPresentCredentialsBeforeChallenge is true
// And also by SGASIS3Request
...
+ (NSThread *)threadForRequest:(SGASIHTTPRequest *)request;
...
@property (assign) SGASIHTTPRequest *mainRequest;
...
@property (assign) id <SGASICacheDelegate> downloadCache;
@property (assign) SGASICachePolicy cachePolicy;
@property (assign) SGASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) SGASIDataDecompressor *dataDecompressor;
